...
impl SignedData {
    ...
    pub fn verify(&self) -> Result<bool> {
        let public_key_bytes = hex::decode(&self.public_key)
            .map_err(|e| ground_core::GroundStationError::Federation(format!("Invalid public key: {}", e)))?;
        
        let public_key_array: [u8; 32] = public_key_bytes.try_into()
            .map_err(|_| ground_core::GroundStationError::Federation("Invalid public key length".to_string()))?;
        
        let public_key = VerifyingKey::from_bytes(&public_key_array)
            .map_err(|e| ground_core::GroundStationError::Federation(format!("Invalid public key: {}", e)))?;
        
        let signature_bytes = hex::decode(&self.signature)
            .map_err(|e| ground_core::GroundStationError::Federation(format!("Invalid signature: {}", e)))?;
        ...
        public_key
            ...
            .map_err(|e| ground_core::GroundStationError::Federation(format!("Verification failed: {}", e)))
            .map(|_| true)
    }
...
impl BitemporalProof {
    ...
    pub fn is_consistent(&self) -> bool {
        ...
        let propagation_delay = (self.last_reception_time - self.first_reception_time).num_seconds();
...
impl AttestationVerifier {
    ...
    pub fn verify_attestation(&self, attestation: &Attestation, station_id: &str) -> Result<bool> {
        let public_key = self.public_keys.get(station_id)
            .ok_or_else(|| ground_core::GroundStationError::Federation("Unknown peer".to_string()))?;
        
        let expected_key = hex::encode(public_key.as_bytes());
        if expected_key != attestation.signed_data.public_key {
            return Err(ground_core::GroundStationError::Federation("Public key mismatch".to_string()));
        }
