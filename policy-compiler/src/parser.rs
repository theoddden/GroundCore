...
impl PolicyParser {
    ...
    pub fn parse_json(json: &str) -> Result<RegulatoryPolicy> {
        let value: Value = serde_json::from_str(json).map_err(|e| {
            ground_core::GroundStationError::InvalidConfiguration(format!(
                "JSON parse error: {}",
                e
            ))
        })?;
    ...
    pub fn parse_value(value: &Value) -> Result<RegulatoryPolicy> {
        let name = value["name"]
            .as_str()
            .ok_or_else(|| {
                ground_core::GroundStationError::InvalidConfiguration(
                    "Missing policy name".to_string(),
                )
            })?
            .to_string();
        ...
        let jurisdiction = value["jurisdiction"]
            ...
            .ok_or_else(|| {
                ground_core::GroundStationError::InvalidConfiguration(
                    "Missing jurisdiction".to_string(),
                )
            })?
            .to_string();
    ...
    fn parse_frequency_band(value: &Value) -> Result<FrequencyBand> {
        let name = value["band"]["name"]
            ...
            .ok_or_else(|| {
                ground_core::GroundStationError::InvalidConfiguration(
                    "Missing band name".to_string(),
                )
            })?
            .to_string();

        let min_frequency = value["band"]["min_frequency"].as_u64().ok_or_else(|| {
            ground_core::GroundStationError::InvalidConfiguration(
                "Missing min frequency".to_string(),
            )
        })?;

        let max_frequency = value["band"]["max_frequency"].as_u64().ok_or_else(|| {
            ground_core::GroundStationError::InvalidConfiguration(
                "Missing max frequency".to_string(),
            )
        })?;
    ...
    fn parse_power_limits(value: &Value) -> Result<PowerLimit> {
        ...
        let power_type = match power_type_str.to_lowercase().as_str() {
            ...
            _ => {
                return Err(ground_core::GroundStationError::InvalidConfiguration(
                    format!("Unknown power type: {}", power_type_str),
                ));
            }
        };
    ...
    fn parse_license_requirement(value: &Value) -> Result<LicenseRequirement> {
        ...
        let license_type = match license_type_str.to_lowercase().as_str() {
            ...
            "amateur" => {
                ...
                crate::rules::LicenseType::Amateur {
            ...
            "specific" => {
                let license_class = value["license_requirement"]["license_class"]
                    ...
                    .ok_or_else(|| {
                        ground_core::GroundStationError::InvalidConfiguration(
                            "Missing license class".to_string(),
                        )
                    })?;
                crate::rules::LicenseType::Specific {
                    license_class: license_class.to_string(),
                }
            }
            _ => {
                return Err(ground_core::GroundStationError::InvalidConfiguration(
                    format!("Unknown license type: {}", license_type_str),
                ));
            }
        };
    ...
    fn parse_coordination_requirement(
        value: &Value,
    ) -> Result<crate::rules::CoordinationRequirement> {
        ...
        let process_str = value["coordination"]["process"].as_str().unwrap_or("none");
        ...
        let process = match process_str.to_lowercase().as_str() {
            ...
            "dynamic" => {
                let response_time = value["coordination"]["response_time_ms"]
                    .as_u64()
                    .unwrap_or(100);
                crate::rules::CoordinationProcess::Dynamic {
                    response_time_ms: response_time,
                }
            }
            "pre" => {
                let notice_days = value["coordination"]["notice_days"].as_u64().unwrap_or(30);
                crate::rules::CoordinationProcess::PreCoordination { notice_days }
            }
            _ => {
                return Err(ground_core::GroundStationError::InvalidConfiguration(
                    format!("Unknown coordination process: {}", process_str),
                ));
            }
        };
