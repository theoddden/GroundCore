...
impl GeneratedTypes {
    ...
    pub fn from_policy(policy: &RegulatoryPolicy) -> Self {
        ...
        for band_rules in &policy.band_rules {
            let coordination_type =
                Self::generate_coordination_type(&band_rules.band, &band_rules.coordination);
    ...
    fn generate_band_type(band: &FrequencyBand) -> Vec<Item> {
        ...
        let doc = format!(
            "{} frequency band ({}-{} MHz)",
            band.name,
            band.lower_hz / 1_000_000,
            band.upper_hz / 1_000_000
        );
    ...
    fn generate_coordination_type(
        band: &FrequencyBand,
        coordination: &crate::rules::CoordinationRequirement,
    ) -> Item {
        let band_suffix = band.name.replace("-", "").replace(" ", "");
        let band_name = format_ident!("Band{}", &band_suffix);
...
mod tests {
    use super::*;
    use crate::rules::{
        CoordinationProcess, CoordinationRequirement, LicenseRequirement, LicenseType, PowerLimit,
        PowerType,
    };
