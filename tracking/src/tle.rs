...
impl TleData {
    ...
    pub fn from_lines(satellite_id: SatelliteId, line1: &str, line2: &str, source: String) -> Result<Self> {
        // Validate TLE format
    ...
    pub fn catalog_number(&self) -> Result<u32> {
        ...
        num_str
            .trim()
            .parse()
            .map_err(|_| ground_core::GroundStationError::Tracking("Invalid catalog number".to_string()))
    }
}
...
fn parse_tle_epoch(s: &str) -> Result<DateTime<Utc>> {
    ...
    let year = if year_part > 56 { 1900 + year_part } else { 2000 + year_part };
