...
impl Satellite {
    ...
    pub fn from_tle(
        ...
        let elements = Elements::from_tle(Some(name.clone()), line1.as_bytes(), line2.as_bytes())
            .map_err(|e| {
            ground_core::GroundStationError::Tracking(format!("SGP4 TLE parse error: {}", e))
        })?;
    ...
    pub fn propagate(&self, time: DateTime<Utc>) -> Result<OrbitalState> {
        let constants = self.constants.as_ref().ok_or_else(|| {
            ground_core::GroundStationError::Tracking("SGP4 constants not initialized".to_string())
        })?;
    ...
    pub fn refresh_tle(&mut self, line1: &str, line2: &str, source: String) -> Result<()> {
        let elements =
            Elements::from_tle(Some(self.name.clone()), line1.as_bytes(), line2.as_bytes())
                .map_err(|e| {
                    ground_core::GroundStationError::Tracking(format!(
                        "SGP4 TLE parse error: {}",
                        e
                    ))
                })?;
