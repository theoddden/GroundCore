...
impl SignalGenerator {
    ...
    fn generate_sine(&mut self) -> (f32, f32) {
        ...
        self.phase = (self.phase + angular_freq / self.sample_rate as f64) % (2.0 * std::f64::consts::PI);
    ...
    fn generate_fsk(&mut self, baud_rate: u32) -> (f32, f32) {
        ...
        let freq_offset = if symbol_index % 2 == 0 { 1000.0 } else { -1000.0 };
        
        let angular_freq = 2.0 * std::f64::consts::PI * (self.carrier_frequency as f64 + freq_offset);
        self.phase = (self.phase + angular_freq / self.sample_rate as f64) % (2.0 * std::f64::consts::PI);
    ...
    fn apply_noise(&mut self, i: f32, q: f32) -> (f32, f32) {
        match self.noise_model {
            ...
            NoiseModel::RicianFading { k_factor_db, doppler_hz } => self.apply_rician(i, q, k_factor_db, doppler_hz),
            NoiseModel::None => (i, q),
        }
    }
    ...
    fn apply_rician(&mut self, i: f32, q: f32, k_factor_db: f32, doppler_hz: f32) -> (f32, f32) {
        ...
        let los = 1.0;
        let scattered = (self.time * fading_rate * 2.0 * std::f64::consts::PI).sin() as f32 * 0.5 + 0.5;
